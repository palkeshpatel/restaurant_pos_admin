...
export default function DecisionGroups() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingItem) {
        await api.put(`/admin/decision-groups/${editingItem.id}`, formData)
      } else {
        await api.post('/admin/decision-groups', formData)
      }
      handleClose()
      fetchGroups()
    } catch (err) {
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography variant="h4">Decision Groups</Typography>
        <Button variant="contained" startIcon={<AddIcon />} onClick={handleOpenAdd}>
          Add New Decision Group
        </Button>
      </Box>
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell>Decisions Count</TableCell>
            ...
            {loading ? (
              ...
              groups.map((group) => (
                <TableRow key={group.id}>
                  ...
                  <TableCell>{group.decisions?.length || 0}</TableCell>
                  <TableCell>
      ...
      <Dialog open={openDialog} onClose={handleClose} maxWidth="sm" fullWidth>
        <DialogTitle>{editingItem ? 'Edit' : 'Add'} Decision Group</DialogTitle>
        <DialogContent>
          {error && (
